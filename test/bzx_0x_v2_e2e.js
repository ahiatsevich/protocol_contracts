...
contract('BZxTest', function(accounts) {
    ...
    context("Off-chain loans", async () => {
        ...
        it("should take sample loan order (as lender2)", async () => {
            ...
            assert.equal(decodeOrders(await bZx.getOrdersForUser.call(lender2, 0, 10)).length, ordersLender2Count + 1);
            assert.equal(decodeOrders(await bZx.getOrdersForUser.call(trader2, 0, 10)).length, ordersTrader2Count + 1);
    ...
    context("0x V2 trading", async () => {
        ...
        it("should sign and verify 0x V2 orders", async () => {
            ...
            assert.isTrue(await exchange_0xV2.isValidSignature.call(
                OrderHash_0xV2_1_onchain,
                OrderParams_0xV2_1["makerAddress"],
    ...
    context("Profits and Interests", async () => {
        it("should withdraw profits (for trader1)", async () => {
            ...
            let traderInitialBalance = await ERC20.at(initialProfit[2]).balanceOf.call(trader1);
            let vaultInitialBalance = await ERC20.at(initialProfit[2]).balanceOf.call(vault.address);
            ...
            let traderFinalBalance = await ERC20.at(initialProfit[2]).balanceOf.call(trader1);
            let vaultFinalBalance = await ERC20.at(initialProfit[2]).balanceOf.call(vault.address);
        ...
        it("should pay lender interest (for trader1)", async () => {
            ...
            assert.isTrue(amount2pay.eq(interest[2]));
    ...
    context("Collateral", async () => {
        ...
        it("should change collateral (for trader1)", async () => {
            ...
            let initialBalance1Trader = await collateralToken1.balanceOf.call(trader1);
            let initialBalance1Vault = await collateralToken1.balanceOf.call(vault.address);

            let initialBalance2Trader = await interestToken1.balanceOf.call(trader1);
            let initialBalance2Vault = await interestToken1.balanceOf.call(vault.address);
            ...
            let finalBalance1Vault = await collateralToken1.balanceOf.call(vault.address);

            let finalBalance2Trader = await interestToken1.balanceOf.call(trader1);
            let finalBalance2Vault = await interestToken1.balanceOf.call(vault.address);
    ...
    context("Loan finalization", async () => {
        ...
        it("should close loan as (lender1/trader1)", async () => {
            let loans = decodeLoanPosition(await bZx.getSingleLoan.call(OrderHash_bZx_1, trader1));
            assert.equal(loans.length, 1);
            assert.isTrue(loans[0].active);

            await bZx.closeLoan(OrderHash_bZx_1, {from: trader1})

            loans = decodeLoanPosition(await bZx.getSingleLoan.call(OrderHash_bZx_1, trader1));
            assert.equal(loans.length, 1);
    ...
    function ensureOrder(order, expectedOrder, expectedHash) {
        assert.equal(expectedOrder.makerAddress, order.maker);
        assert.equal(expectedOrder.loanTokenAddress, order.loanTokenAddress);
        assert.equal(expectedOrder.interestTokenAddress, order.interestTokenAddress);
        assert.equal(expectedOrder.collateralTokenAddress, order.collateralTokenAddress);
        assert.equal(expectedOrder.feeRecipientAddress, order.feeRecipientAddress);
        assert.equal(expectedOrder.oracleAddress, order.oracleAddress);
        assert.isTrue(BigNumber(expectedOrder.loanTokenAmount).eq(BigNumber(order.loanTokenAmount)));
        assert.isTrue(BigNumber(expectedOrder.interestAmount).eq(BigNumber(order.interestAmount)));
